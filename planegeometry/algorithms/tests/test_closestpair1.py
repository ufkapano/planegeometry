...
class TestClosestPair(unittest.TestCase):
    ...
    def test_closest_pair(self):
        algorithm = ClosestPairSweepLine(self.point_list)
        algorithm.run()
        self.assertEqual(algorithm.closest_pair, self.result)

        self.assertEqual(sorted(ClosestPairSweepLine(self.points1).run()),
            sorted((Point(-2, 5), Point(-1, 7))))
        self.assertEqual(sorted(ClosestPairSweepLine(self.points2).run()),
            sorted((Point(1, 3), Point(4, 3))))
        self.assertEqual(sorted(ClosestPairSweepLine(self.points3).run()),
            sorted((Point(1, 3), Point(4, 3))))

    def test_exceptions(self):
        self.assertRaises(ValueError, ClosestPairSweepLine, [Point(1, 1)])
